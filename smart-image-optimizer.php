...
class SmartImageOptimizer {
    ...
    public function __construct() {
        ...
        foreach ( [
            'sio_get_images','sio_get_stats','sio_save_settings',
            'sio_ai_name','sio_rename_single','sio_resize_single',
            'sio_bulk','sio_scan_unused','sio_delete_unused',
            'sio_scan_cats','sio_apply_cats','sio_apply_cat_folders',
            'sio_scan_refs','sio_fix_refs',
        ] as $a ) add_action( "wp_ajax_{$a}", [ $this, "ajax_{$a}" ] );
    }

    /* ── Taxonomy for media folders ── */
    public static function reg_tax() {
        register_taxonomy( 'media_cat', 'attachment', [
            'label' => 'Media Category', 'public' => false,
            'show_ui' => true, 'show_in_menu' => false,
            'rewrite' => false, 'hierarchical' => true,
        ] );
    }

    /* ── Menu ── */
    public function menu() {
        add_media_page( 'Smart Image Optimizer', '🖼 Smart Optimizer',
            'manage_options', 'smart-image-optimizer', [ $this, 'page' ] );
    }

    /* ── Scripts ── */
    public function enqueue( $hook ) {
        if ( strpos( $hook, 'smart-image-optimizer' ) === false ) return;
        wp_enqueue_media();
    ...
    private function cfg() : array {
        return wp_parse_args( get_option( $this->opt, [] ), [
            'ai_provider' => 'groq', 'groq_key' => '', 'gemini_key' => '',
            'api_key' => '', 'ai_model' => 'claude-haiku-4-5-20251001',
            'ai_language' => 'en', 'ai_context' => '',
        ] );
    }
    ...
    public function ajax_sio_get_images() {
        ...
        $args = [ 'post_type'=>'attachment','post_mime_type'=>'image','post_status'=>'inherit',
                  'posts_per_page'=>$pp,'paged'=>$page,'orderby'=>'date','order'=>'DESC' ];
        if ( $src ) $args['s'] = $src;
        $q = new WP_Query( $args );
        $out = [];
        foreach ( $q->posts as $p ) {
            ...
            if ( $flt==='bad'   && !$bad   ) continue;
            if ( $flt==='large' && $kb<300 ) continue;
            $out[] = [ 'id'=>$p->ID,'filename'=>$fn,'thumb'=>wp_get_attachment_image_url($p->ID,'thumbnail'),
                       'width'=>$meta['width']??0,'height'=>$meta['height']??0,'size_kb'=>$kb,'bad_name'=>$bad ];
        }
    ...
    public function ajax_sio_get_stats() {
        ...
        $bad = 0;
        foreach ($ids as $id) if (($f=get_attached_file($id)) && $this->bad_name(basename($f))) $bad++;
    ...
    public function ajax_sio_rename_single() {
        ...
        if (!$id||!$nm) wp_send_json_error('Missing data');
    ...
    public function ajax_sio_resize_single() {
        check_ajax_referer( 'sio_nonce', 'nonce' );
        $id=intval($_POST['id']??0); $w=intval($_POST['width']??0); $h=intval($_POST['height']??0);
        $m=sanitize_text_field($_POST['mode']??'max_width');
        if (!$id||(!$w&&!$h)) wp_send_json_error('Missing data');
    ...
    public function ajax_sio_bulk() {
        ...
        $s   = $this->cfg();
        $ctx = $s['ai_context']??''; $lng = $s['ai_language']??'en';
        $out = ['success'=>0,'failed'=>0,'skipped'=>0,'replaced'=>0,'details'=>[]];

        foreach ($ids as $id) {
            $f = get_attached_file($id);
            if (!$f||!file_exists($f)) { $out['skipped']++; $out['details'][]="⏭ #{$id}: file not found"; continue; }
            $fn = basename($f); $detail = "#{$id} [{$fn}]";
            try {
                if ($op==='rename'||$op==='both') {
                    $slug = $this->ai_call($f,$ctx,$lng,'name');
                    if (is_wp_error($slug)) { $out['failed']++; $out['details'][]="❌ {$detail}: ".$slug->get_error_message(); continue; }
                    $nm   = ($pfx?sanitize_title($pfx).'-':'').$slug.'-'.$id;
                    $ren  = $this->do_rename($id,$nm);
                    if (is_wp_error($ren)) { $out['failed']++; $out['details'][]="❌ {$detail}: ".$ren->get_error_message(); continue; }
                ...
                $out['success']++; $out['details'][]="✅ {$detail}";
            } catch(\Exception $e){ $out['failed']++; $out['details'][]="❌ #{$id}: ".$e->getMessage(); }
        }
    ...
    public function ajax_sio_scan_unused() {
        ...
        $unused=[];
        foreach ($ids as $id) {
            if (!$this->is_unused($id)) continue;
            $f=get_attached_file($id); $kb=($f&&file_exists($f))?round(filesize($f)/1024,1):0;
    ...
    public function ajax_sio_delete_unused() {
        ...
        $ids=array_map('intval',(array)($_POST['ids']??[]));
        $del=$fail=0;
        foreach ($ids as $id) { if ($this->is_unused($id)) { wp_delete_attachment($id,true)?$del++:$fail++; } }
        wp_send_json_success(['deleted'=>$del,'failed'=>$fail]);
    }

    private function is_unused(int $id): bool {
        global $wpdb;
        $f=get_attached_file($id); if(!$f) return false;
        $fn=basename($f); if(!$fn) return false;
        // featured image?
        if ($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key='_thumbnail_id' AND meta_value=%d",$id))) return false;
        $lk='%'.$wpdb->esc_like($fn).'%';
        if ($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->posts} WHERE post_content LIKE %s AND post_status!='inherit'",$lk))) return false;
        if ($wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_value LIKE %s AND meta_key!='_thumbnail_id'",$lk))) return false;
        return true;
    }
    ...
    public function ajax_sio_scan_cats() {
        ...
        $ids=array_map('intval',(array)($_POST['ids']??[]));
        if (empty($ids)) wp_send_json_error('No images selected');
        $s=$this->cfg(); $ctx=$s['ai_context']??''; $lng=$s['ai_language']??'en';
        $cats=[]; $details=[];
        foreach ($ids as $id) {
            $f=get_attached_file($id); if(!$f||!file_exists($f)) continue;
            $cat=$this->ai_call($f,$ctx,$lng,'category');
            if (is_wp_error($cat)) { $details[]="❌ #{$id}: ".$cat->get_error_message(); continue; }
            $cats[$cat][]=$id;
    ...
    public function ajax_sio_apply_cats() {
        ...
        foreach ($cats as $slug=>$ids) {
            ...
            if (!$term) $term = wp_insert_term(ucwords(str_replace('-',' ',$slug)),'media_cat',['slug'=>$slug]);
            ...
            if (!file_exists($cat_dir)) wp_mkdir_p($cat_dir);
            ...
            foreach ((array)$ids as $id) {
                ...
                if (!$old_file||!file_exists($old_file)) continue;
                ...
                // Move main file
                if (!@rename($old_file, $new_file)) { $failed[]="#{$id}: could not move"; continue; }
    ...
    public function ajax_sio_apply_cat_folders() {
        ...
        foreach ( $cats as $slug => $ids ) {
            ...
            foreach ( (array) $ids as $id ) {
                ...
                if ( ! $term ) $term = wp_insert_term( ucwords( str_replace( '-', ' ', $folder_name ) ), 'media_cat', [ 'slug' => $folder_name ] );
                if ( ! is_wp_error( $term ) ) {
                    $tid = is_array( $term ) ? $term['term_id'] : $term;
    ...
    public function ajax_sio_scan_refs() {
        ...
        if (!$fn) wp_send_json_error('Filename required');
    ...
    public function ajax_sio_fix_refs() {
        check_ajax_referer('sio_nonce','nonce');
        if (!current_user_can('manage_options')) wp_send_json_error();
        $o=sanitize_text_field($_POST['old']??''); $n=sanitize_text_field($_POST['new']??'');
        if (!$o||!$n) wp_send_json_error('Missing data');
    ...
    private function ai_call(string $file,string $ctx,string $lang,string $task) {
        $s=$this->cfg(); $p=$s['ai_provider']??'groq';
        return match($p){
    ...
    private function slug(string $raw) {
        $s=trim(strtolower($raw)); $s=preg_replace('/[^a-z\-]/','', $s);
        $s=preg_replace('/-{2,}/','-',$s); $s=trim($s,'-');
    ...
    private function prep_image(string $file) {
        ...
        $ext=strtolower(pathinfo($file,PATHINFO_EXTENSION)); $mime=$em[$ext]??null;
        if (!$mime) return new \WP_Error('unsupported',"Unsupported: {$ext}");
        ...
        if (!is_wp_error($ed)){
            ...
            if (($sz['width']??0)>1024||($sz['height']??0)>1024){
                ...
                $sv=$ed->save($tmp);
                if (!is_wp_error($sv)&&file_exists($tmp)){$d=base64_encode(file_get_contents($tmp));@unlink($tmp);return['data'=>$d,'mime'=>$mime];}
            }
        }
    ...
    private function ai_groq(string $f,string $ctx,string $lang,string $task) {
        $s=$this->cfg(); $key=trim($s['groq_key']??'');
        if (!$key) return new \WP_Error('no_key','Groq API key missing — go to Settings');
        $img=$this->prep_image($f); if (is_wp_error($img)) return $img;
        $body=wp_json_encode(['model'=>'meta-llama/llama-4-scout-17b-16e-instruct','max_tokens'=>60,'temperature'=>0.2,
        ...
        if (is_wp_error($r)) return new \WP_Error('api','Groq failed: '.$r->get_error_message());
        $code=wp_remote_retrieve_response_code($r); $json=json_decode(wp_remote_retrieve_body($r),true);
        if ($code!==200) return new \WP_Error('api','Groq error: '.($json['error']['message']??"HTTP {$code}"));
        return $this->slug($json['choices'][0]['message']['content']??'');
    }

    private function ai_gemini(string $f,string $ctx,string $lang,string $task) {
        $s=$this->cfg(); $key=trim($s['gemini_key']??'');
        if (!$key) return new \WP_Error('no_key','Gemini API key missing — go to Settings');
        $img=$this->prep_image($f); if (is_wp_error($img)) return $img;
        ...
        foreach (['gemini-1.5-flash-8b','gemini-1.5-flash','gemini-2.0-flash-lite','gemini-2.0-flash'] as $model) {
            ...
            $code=wp_remote_retrieve_response_code($r); $json=json_decode(wp_remote_retrieve_body($r),true);
            if ($code===200) return $this->slug($json['candidates'][0]['content']['parts'][0]['text']??'');
    ...
    private function ai_anthropic(string $f,string $ctx,string $lang,string $task) {
        $s=$this->cfg(); $key=trim($s['api_key']??'');
        if (!$key) return new \WP_Error('no_key','Anthropic API key missing — go to Settings');
        $img=$this->prep_image($f); if (is_wp_error($img)) return $img;
        $body=wp_json_encode(['model'=>$s['ai_model']??'claude-haiku-4-5-20251001','max_tokens'=>60,
            'messages'=>[['role'=>'user','content'=>[
        ...
        if (is_wp_error($r)) return new \WP_Error('api','Anthropic failed: '.$r->get_error_message());
        $code=wp_remote_retrieve_response_code($r); $json=json_decode(wp_remote_retrieve_body($r),true);
        if ($code!==200) return new \WP_Error('api','Anthropic error: '.($json['error']['message']??"HTTP {$code}"));
    ...
    private function do_rename(int $id,string $new_name) {
        ...
        if (!$old||!file_exists($old)) return new \WP_Error('nf','File not found on server');
        $dir=dirname($old); $ext=strtolower(pathinfo($old,PATHINFO_EXTENSION));
        $os=pathinfo($old,PATHINFO_FILENAME);
        $ns=trim(preg_replace('/\.'.preg_quote($ext,'/').'$/i','',sanitize_file_name($new_name)),'-');
        $ud=wp_upload_dir(); $ou=wp_get_attachment_url($id);
        $rm=[]; $um=[];
        $ns2=$this->uniq_stem($dir,$ns,$ext,$old); $nfn=$ns2.'.'.$ext; $nf=$dir.'/'.$nfn;
        if ($nf!==$old){$rm[$old]=$nf;$um[basename($old)]=$nfn;}
        $meta=wp_get_attachment_metadata($id); $nsz=[];
        if (!empty($meta['sizes'])) foreach ($meta['sizes'] as $sk=>$sd){
            $ot=$dir.'/'.$sd['file']; if (!file_exists($ot)) continue;
            $te=strtolower(pathinfo($sd['file'],PATHINFO_EXTENSION));
            $suf=preg_replace('/^'.preg_quote($os,'/').'/','',$pf=pathinfo($sd['file'],PATHINFO_FILENAME));
            $nt=$ns2.$suf.'.'.$te;
            $rm[$ot]=$dir.'/'.$nt; $um[$sd['file']]=$nt; $nsz[$sk]=array_merge($sd,['file'=>$nt]);
        }
        foreach ($rm as $from=>$to) if ($from!==$to&&!@rename($from,$to)) return new \WP_Error('rf','Cannot rename: '.basename($from));
        ...
        if ($meta){$meta['file']=str_replace($ud['basedir'].'/','',$nf);foreach($nsz as $k=>$v)$meta['sizes'][$k]=$v;wp_update_attachment_metadata($id,$meta);}
    ...
    private function do_resize(int $id,int $w,int $h,string $mode) {
        $f=get_attached_file($id); if (!$f||!file_exists($f)) return new \WP_Error('nf','File not found');
        $meta=wp_get_attachment_metadata($id); $ow=$meta['width']??0; $oh=$meta['height']??0;

        // No skipping — allow both upscale and downscale in all modes

        $ed=wp_get_image_editor($f); if (is_wp_error($ed)) return $ed;
        $ob=filesize($f);
        ...
        if (!$src) return new \WP_Error('gd','Cannot open image with GD');
        ...
        $saved=false;
        if ($ext==='png')       $saved=imagepng($dst,$f,8);
        elseif ($ext==='webp')  $saved=imagewebp($dst,$f,85);
        else                    $saved=imagejpeg($dst,$f,85);

        imagedestroy($src);
        imagedestroy($dst);

        if (!$saved) return new \WP_Error('gd_save','GD could not save the image');
    ...
    private function db_replace_map(array $map): int {
        global $wpdb;
        $base=trailingslashit(wp_upload_dir()['baseurl']); $pairs=[];
        foreach ($map as $o=>$n){ if($o===$n)continue; $pairs[$o]=$n; $pairs[$base.$o]=$base.$n; }
        if (empty($pairs)) return 0;
        $total=0; $skip=['rewrite_rules','cron','auth_key','secure_auth_key'];
        ...
        wp_cache_flush(); return $total;
    }

    private function find_refs(string $fn): array {
        global $wpdb; $lk='%'.$wpdb->esc_like($fn).'%'; $refs=[];
        foreach (['posts'=>"SELECT ID,post_title FROM {$wpdb->posts} WHERE post_content LIKE %s AND post_status!='inherit' LIMIT 20",
                  'postmeta'=>"SELECT post_id as ID,meta_key as post_title FROM {$wpdb->postmeta} WHERE meta_value LIKE %s LIMIT 20",
                  'options'=>"SELECT option_id as ID,option_name as post_title FROM {$wpdb->options} WHERE option_value LIKE %s LIMIT 10"] as $t=>$sql){
            $rows=$wpdb->get_results($wpdb->prepare($sql,$lk));
            if ($rows) $refs[]=['table'=>$t,'count'=>count($rows),'rows'=>array_map(fn($r)=>"#{$r->ID} — {$r->post_title}",array_slice($rows,0,5))];
        }
        return $refs;
    }

    private function uniq_stem(string $dir,string $stem,string $ext,string $keep=''): string {
        if (!file_exists("$dir/$stem.$ext")||"$dir/$stem.$ext"===$keep) return $stem;
        for($i=1;file_exists("$dir/{$stem}-{$i}.{$ext}");$i++); return "{$stem}-{$i}";
    }
    ...
    public function page() {
        $s=$this->cfg(); $pv=$s['ai_provider']??'groq';
?>
<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"></head><body>
<div id="sio-app" class="wrap">
  <div class="sio-header">
    <div><h1>🖼 Smart Image Optimizer <span class="vbadge">v4.0 · AI Vision</span></h1>
    <p>Rename · Resize · Delete unused · Smart categorize — all AI-powered</p></div>
  ...
  <div class="pane active" id="tab-browse">
    <div class="toolbar">
      ...
      <select id="filter"><option value="all">All images</option><option value="bad">Bad filenames</option><option value="large">Large (+300KB)</option></select>
      <select id="per-page"><option value="24">24/page</option><option value="48">48/page</option><option value="96">96/page</option></select>
    ...
    <div class="panel">
      ...
      <div class="op-row">
        <label class="op-card active"><input type="radio" name="op" value="rename" checked>
          ...
          <span>AI generates descriptive filename from image content</span></label>
        <label class="op-card"><input type="radio" name="op" value="resize">
          <div class="op-ico">📐</div><strong>Resize only</strong>
          <span>Compress to max dimensions, no renaming</span></label>
        <label class="op-card"><input type="radio" name="op" value="both">
          <div class="op-ico">⚡</div><strong>Rename + Resize</strong>
          <span>Do both in a single pass</span></label>
      </div>

      <div id="ren-opts">
        <div class="form-row">
          <div class="fg"><label>Prefix (optional)</label><input type="text" id="b-prefix" placeholder="e.g. product, hero" /></div>
          <div class="fg"><label>Language</label><select id="b-lang"><option value="en">English (SEO recommended)</option><option value="ar">Arabic → English</option><option value="auto">Auto-detect</option></select></div>
          <div class="fg"><label>Site context (optional)</label><input type="text" id="b-ctx" placeholder="e.g. fashion store, restaurant" /></div>
        </div>
      </div>
      <div id="res-opts" style="display:none">
        <div class="form-row">
          ...
          <div class="fg" id="b-h-wrap"><label>Target height (px)</label><input type="number" id="b-maxh" value="800" min="50" /></div>
          <div class="fg" id="b-w-wrap" style="display:none"><label>Max width (px)</label><input type="number" id="b-maxw" value="1920" min="100" /></div>
        </div>
      </div>
      ...
      <div id="bulk-prog" style="display:none">
        <div class="prog-wrap"><div class="prog-bar"><div id="prog-fill"></div></div><span id="prog-pct">0%</span></div>
  ...
  <div class="pane" id="tab-unused">
    <div class="panel">
      <h2>🗑 Unused Images</h2>
      <p class="hint">Scans for images not referenced in any post, page, widget, or custom field. Review carefully before deleting.</p>
  ...
  <div class="pane" id="tab-cats">
    <div class="panel">
      ...
      <p class="hint">The AI analyzes each selected image and groups similar ones into categories. Categories are saved as a WordPress Media taxonomy so you can filter by them in the Media Library.</p>
      <div class="notice info" style="margin-bottom:16px;">📂 <strong>Real folders (default ON):</strong> Images are physically moved to <code>uploads/categories/{name}/</code> — all links updated automatically.<br>💡 To see these folders in the Media Library sidebar, install the free <strong>FileBird</strong> plugin.</div>
      ...
      <div id="cat-prog" style="display:none;margin-top:16px;">
        <div class="prog-wrap"><div class="prog-bar"><div id="cat-fill"></div></div><span id="cat-pct">0%</span></div>
  ...
  <div class="pane" id="tab-audit">
    <div class="panel">
      ...
      <div class="audit-row">
        <div class="fg"><label>Old filename</label><input type="text" id="a-old" placeholder="old-name.jpg" /></div>
        <div class="fg"><label>New filename</label><input type="text" id="a-new" placeholder="new-name.jpg" /></div>
  ...
  <div class="pane" id="tab-settings">
    <div class="panel">
      ...
      <div class="prov-row">
        <label class="prov-card <?= $pv==='groq'?'active':'' ?>"><input type="radio" name="prov" value="groq" <?= checked($pv,'groq',false) ?>>
          <span class="badge free">Free ✅</span><div class="pico">⚡</div><strong>Groq</strong><span>Free globally · No card</span></label>
        <label class="prov-card <?= $pv==='gemini'?'active':'' ?>"><input type="radio" name="prov" value="gemini" <?= checked($pv,'gemini',false) ?>>
          <span class="badge free">Free*</span><div class="pico">🟢</div><strong>Gemini</strong><span>Free in some regions</span></label>
        <label class="prov-card <?= $pv==='anthropic'?'active':'' ?>"><input type="radio" name="prov" value="anthropic" <?= checked($pv,'anthropic',false) ?>>
          <span class="badge paid">Paid</span><div class="pico">🟣</div><strong>Claude</strong><span>~$0.001/image · Best accuracy</span></label>
      </div>
      <div class="sfields">
        <div class="sf p-groq" style="<?= $pv!=='groq'?'display:none':'' ?>">
          <label>🔑 Groq API Key <span class="badge free">Free</span></label>
          <div class="key-row"><input type="password" id="s-groq" value="<?= esc_attr($s['groq_key']??'') ?>" placeholder="gsk_..."/><button class="btn secondary tpw">👁</button></div>
          <small>Get free key → <a href="https://console.groq.com/keys" target="_blank"><strong>console.groq.com/keys</strong></a> (sign in with Google, no credit card)</small>
        </div>
        <div class="sf p-gemini" style="<?= $pv!=='gemini'?'display:none':'' ?>">
          <label>🔑 Google Gemini API Key <span class="badge free">Free</span></label>
          <div class="key-row"><input type="password" id="s-gemini" value="<?= esc_attr($s['gemini_key']??'') ?>" placeholder="AIzaSy..."/><button class="btn secondary tpw">👁</button></div>
          <small>Get key → <a href="https://aistudio.google.com/app/apikey" target="_blank"><strong>aistudio.google.com/app/apikey</strong></a></small>
        </div>
        <div class="sf p-anthropic" style="<?= $pv!=='anthropic'?'display:none':'' ?>">
          <label>🔑 Anthropic API Key</label>
          <div class="key-row"><input type="password" id="s-claude" value="<?= esc_attr($s['api_key']??'') ?>" placeholder="sk-ant-api03-..."/><button class="btn secondary tpw">👁</button></div>
          <small>Get key → <a href="https://console.anthropic.com/settings/keys" target="_blank">console.anthropic.com</a> (requires billing)</small>
          <div class="fg" style="margin-top:10px;max-width:280px;"><label>Model</label>
            <select id="s-model"><option value="claude-haiku-4-5-20251001" <?= selected($s['ai_model'],'claude-haiku-4-5-20251001',false) ?>>Claude Haiku (fast ✅)</option><option value="claude-sonnet-4-6" <?= selected($s['ai_model'],'claude-sonnet-4-6',false) ?>>Claude Sonnet (accurate)</option></select></div>
        </div>
        <div class="form-row" style="margin-top:16px;">
          <div class="fg"><label>🌍 Filename language</label><select id="s-lang"><option value="en" <?= selected($s['ai_language'],'en',false) ?>>English (recommended)</option><option value="ar" <?= selected($s['ai_language'],'ar',false) ?>>Arabic → English</option><option value="auto" <?= selected($s['ai_language'],'auto',false) ?>>Auto-detect</option></select></div>
          <div class="fg"><label>📌 Site context (improves accuracy)</label><input type="text" id="s-ctx" value="<?= esc_attr($s['ai_context']??'') ?>" placeholder="e.g. online fashion store, restaurant menu"/></div>
        </div>
      </div>
      ...
      <div class="info-grid" style="margin-top:28px;">
        <div class="ic green"><h3>⚡ Groq — Free &amp; Global</h3><p>Completely free, no credit card, works worldwide. Powered by Llama 4 Vision. <a href="https://console.groq.com/keys" target="_blank">console.groq.com/keys</a></p></div>
        <div class="ic"><h3>💰 Claude cost estimate</h3><p>1000 images ≈ <strong>$0.50–$1.00</strong> with Claude Haiku depending on file size.</p></div>
        <div class="ic warn"><h3>⚠ Before running</h3><p>Backup your database first. Test on 5 images before bulk runs. Rename &amp; delete operations cannot be undone.</p></div>
      </div>
    </div>
  </div>

  <!-- MODAL -->
  <div id="modal" class="modal" style="display:none"><div class="mbox"><button class="mclose">✕</button><div id="mbody"></div></div></div>
</div>
</body></html>
<?php } }
